...
- "Prefer `size-*` over `w-* h-*`", "No `space-x-*`/`space-y-*`, use `gap-*`", "Use `cn()` for conditional classes", "No manual z-index on overlay components.", all ([skills/shadcn/rules/styling.md](https://github.com/shadcn-ui/ui/blob/main/skills/shadcn/rules/styling.md))
