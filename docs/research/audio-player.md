...
controls are near or within the viewport". That is a real mitigation for the N-element architecture —
but the shared-element architecture (§2) makes it moot.
...
"Pause, pressed". Include the Song title in the name when the button is icon-only —
`aria-label="Play — {song.title}"` — so the twelve buttons are distinguishable in a rotor list.
...
([wavesurfer.js README](https://github.com/katspaugh/wavesurfer.js)), and the same class of problem —
inaccurate duration and imprecise seeking from a missing or wrong Xing header — affects any
...
- `disableLocalStorage: true` is required when an S3/R2 adapter owns the bytes. The docs warn that
  with it set, "your admin panel's thumbnails will be broken as you will not have stored a file" —
  irrelevant for audio, which has no thumbnail anyway.
