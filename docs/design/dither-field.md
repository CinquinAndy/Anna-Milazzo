...
L0 is the untouched hero ground and is **never painted** in the sense of being a decision, it is the "off" cell. The whole field is one colour at three depths: hue drift across the
...
when the effect runs (`color` inherits and resolves to the body's near-black or to white, either way the field would paint over the copy), a future retheme, and any caller who mounts
...
was and slows to the room's tempo. It never freezes, nothing on this page freezes mid-motion, and it is legibly different from both other states in one line of code.
...
`--field-baseline` in `globals.css`, not the amplitudes, not the reach, not the palette, until it passes at all six combinations. It is the one number in this design that is measured
...
The field's height is anchored to the foot of the section, but the copy's position is not, it is centred in whatever `md:my-auto` leaves. Three routine events move one without the
