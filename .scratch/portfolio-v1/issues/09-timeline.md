...
- **The seek thumb now follows from the engine's animation frame**, alongside
  `--playhead`. It had been driven from `state.positionSeconds`, which nothing advanced, so the thumb would have sat at zero for the whole Song. Every per-frame DOM write now
