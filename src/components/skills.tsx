				...
				<div className="shell relative">
					...
					<div className="grid items-center gap-14 lg:grid-cols-[1fr_auto] lg:gap-20">
						{/* Generous and uneven: the tags are stuck on, and things stuck on do not sit at
						    a uniform pitch. */}
						<ul className="flex flex-wrap gap-x-6 gap-y-8 sm:gap-x-10 sm:gap-y-10">
							{entries.map((entry, index) => (
								<li
									key={entry.id ?? entry.name}
									style={{ transform: `rotate(${TAG_TILTS[index % TAG_TILTS.length]}deg)` }}
									className={`border-brutal border-border ${TAG_FILLS[index % TAG_FILLS.length]} px-6 py-4 font-mono text-base text-foreground shadow-lg sm:text-lg`}
								>
									{entry.name}
								</li>
							))}
