...
const FILLS = ['bg-card', 'bg-secondary', 'bg-accent'] as const
...
export function Timeline({ timeline, scrollLabel }: { timeline: Home['timeline']; scrollLabel: string }) {
	...
	return (
		<section className="border-b-brutal border-border bg-background px-5 py-14 sm:px-8 md:py-20" data-timeline>
			<div className="mx-auto max-w-6xl">
				...
				<section className="mt-10 overflow-x-auto pb-4" tabIndex={0} aria-label={scrollLabel} data-timeline-scroller>
					<ol className="flex min-w-max list-none items-stretch gap-0 border-t-brutal border-border p-0 pt-0">
						{entries.map((entry, index) => (
							<li key={entry.id ?? entry.period} className="relative flex min-w-[14rem] max-w-[18rem] flex-col">
								...
								<div aria-hidden="true" className="h-4 w-1 bg-border" />
								<div
									className={`flex h-full flex-col border-brutal border-border ${FILLS[index % FILLS.length]} -ml-[2px] p-4`}
								>
